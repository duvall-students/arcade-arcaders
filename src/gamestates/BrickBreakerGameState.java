...
public abstract class BrickBreakerGameState extends GameState {
	...
	protected String pathToScoreFile = "resources/bb_high_score";

	// Constructor that calls super constructor then adds in brickbreaker elements
	public BrickBreakerGameState(int screenWidth, int screenHeight, int numberOfBrickRows, int brickOdds,
			int oddsOfUnbreakableBrick) {
	...
	// Inherited from GameState and makes basic frame checks for the brickbreaker
	// game
	@Override
		...
		for (int x = 1; x <= numberOfBrickRows; x++) {
			for (int i = 0; i < this.screenWidth / Brick.brickWidth; i++) {
				if (randomVal.nextInt(maxOddsOfBrick) < brickOdds) {
					...
					UnbreakableBrick brickCreated = new UnbreakableBrick(i * Brick.brickWidth, Brick.brickHeight * x,
							this);
					gameTargets.add(brickCreated);
					root.getChildren().add(brickCreated.getNode());
				}
